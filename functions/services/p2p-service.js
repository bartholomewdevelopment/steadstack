...
const createReceipt = async (tenantId, receiptData, userId, userRoles) => {
  ...
  for (const line of receiptData.lines) {
    const poLine = po.lineItems.find(pl => pl.lineNumber === line.poLineNumber);
    if (!poLine) {
    ...
    if (line.qtyReceived > remainingQty) {
      throw new Error(`Cannot receive ${line.qtyReceived} for line ${line.poLineNumber}. Only ${remainingQty} remaining.`);
    }

    const unitCost = line.unitCost ?? poLine.unitPrice;
    const lineTotalCost = line.qtyReceived * unitCost;
...
const postReceipt = async (tenantId, receiptId, userId) => {
  ...
  try {
    ...
    await postingEngine.processEvent(tenantId, result.eventId);
  } catch (processError) {
    console.error('Failed to process receipt event:', processError);
    // Don't throw - the receipt is posted, event can be processed later
  }
