...
router.post('/receipts/:id/post', async (req, res) => {
  try {
    ...
    // Check if inventory processing failed
    if (result.processingError) {
      console.error('Receipt posted but inventory processing failed:', result.processingError);
      return res.status(207).json({
        success: true,
        partial: true,
        message: 'Receipt posted but inventory update failed. Please contact support.',
        data: result,
      });
    }
...
/**
 * POST /api/purchasing/reprocess-failed-receipts
 * Reprocess failed receipt events to fix inventory
 */
router.post('/reprocess-failed-receipts', async (req, res) => {
  try {
    const { tenantId } = await getUserContext(req);
    const results = await p2pService.reprocessFailedReceiptEvents(tenantId);

    res.json({
      success: true,
      message: `Reprocessed ${results.reprocessed} events: ${results.succeeded} succeeded, ${results.failed} failed`,
      data: results,
    });
  } catch (error) {
    console.error('Error reprocessing failed receipts:', error);
    res.status(500).json({ success: false, message: error.message });
  }
});
